...
def plot_soc(times, prices, socs, path_to_fig, price_flag=True, soc_flag=True, labels=None):
    ...
    if soc_flag:
        ...
        if len(socs) == 2:
            ...
            lns1 = ax1.plot([times[0] - default.FIVE_MIN] + list(times), [50] + list(socs), label='SOC', color=default.BROWN)
...
def plot_power(times, prices, powers, bid_type, path_to_fig):
    ...
    if len(powers) == 2:
        ...
        lns1 = ax1.plot(times, powers, label=bid_type, color=default.BROWN)
        lns = lns1
...
if __name__ == '__main__':
    ...
    # u1 = 'DER None Elastic Bilevel Hour'
    u1 = 'DER None Integrated Hour No-losses'
    battery = Battery(e, int(e / 3 * 2), usage=u1)
    start = datetime.datetime(2021, 7, 18, 4, 30)
    times, socs, _, _, prices, _ = read_battery_optimisation(battery.bat_dir / f'{default.get_case_datetime(start)}.csv')  # Rolling horizon
    ...
    u2 = 'DER None Integrated Hour No-losses'
    # u2 = 'DER None Elastic Bilevel Hour'
    battery = Battery(0, int(0 / 3 * 2), usage=u2)
    # _, _, prices2, _ = read_der(0, start, battery.bat_dir)  # First horizon
    _, _, _, _, prices2, _ = read_battery_optimisation(battery.bat_dir / f'{default.get_case_datetime(start)}.csv')
