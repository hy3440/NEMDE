...
# Price
price_dict = {}
period_rrp_record = []  # Average prices over 6 intervals as period price record
...
def schedule(i, t, battery):
    ...
    if i == START:
        ...
        current_E = E_record[i] if battery.name == 'Dalrymple' else battery.Emax * 0.5
    else:
...
def main(battery):
    ...
    for i in range(TOTAL_INTERVALS):
        ...
        interval_rrp_record.append(extract_dispatch(time + FIVE_MIN, battery.region))  # Get energy price and FCAS prices for interval
